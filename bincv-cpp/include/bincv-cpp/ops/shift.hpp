...
/// @file shift.hpp
/// @brief Horizontal and vertical shifts with OpenCV border semantics (T2.3, T2.4).
///
/// The primitive morphology (T3.3) and the binarized derivative (T3.5) are both
/// built out of, and -- as TASKS.md T2.3 says in as many words -- the easiest
...
///
/// This matches ARCHITECTURE 7.4, which writes the derivative's rising edge as
/// `(src >> 1) & ~(src << 1)` and TASKS.md T3.5 writes the same expression as
...
///
/// TASKS.md T3.3 builds dilate as an OR of shifted copies and erode as an AND of
...
/// defaulting to BORDER_CONSTANT / false -- the plain zero fill T2.3 specifies for
/// the bare three-argument call, and what T3.5's derivative wants.
///
...
///  3. **No allocation, no throw** (ARCHITECTURE 5.3). A 2-D shift is one pass
...
///
/// This is the half of D-11 that applies, and the exact-alias half deliberately
/// does NOT extend here. D-11 permits `dst` to be exactly a source because the
/// T2.2 kernels are POINTWISE IN THE WORD INDEX -- word i of the destination is
...
/// row order that works for every (dy, BorderType), and D-11 forbids the temporary
/// that would make one unnecessary.
///
/// So: exactly one supported relationship, checked per row rather than by bounding
/// box (D-5 -- interleaved row bands and left/right column tiles share a buffer
...
// is written in terms of. Shared with ops/logic.hpp so that D-11 has one copy.
...
///       precondition here (ARCHITECTURE 5.3).
...
///       which T2.4 requires to be correct, not merely defined -- costs the loop
...
///       (TASKS.md T2.3: "correct scalar first"); the branch inside
...
///        (morphology, T3.3) be bit-exact in turn.
///
...
///       alongside the four directional ones -- T3.3 shifts by a structuring
...
///        zero-filled right edge TASKS.md T2.3 specifies for the three-argument
...
///        **API TIER 3** by name, TIER 1 border semantics. See shift().
/// @note No bit manipulation at all (T2.4): a vertical shift is a row-index remap,
