...
/// @file denoise.hpp
/// @brief The reference pipeline's three-pixel median filter (T3.1).
...
/// vocabulary that is BORDER_CONSTANT with value `false` (D-12), which is also
...
///                                          = maj3(p1, p2, p3)
///
/// which is ARCHITECTURE 7.1's expression and ops/bitslice.hpp's `maj3`. The
...
/// drop-in promise that this operation cannot keep (ARCHITECTURE 5.1), so the
...
///              T2.4), and the right-neighbour is the current row's words shifted
...
///
/// ---------------------------------------------------------------------------
/// WHAT THE KERNEL PROMISES (the ops/ contract, D-5 and D-11)
///
///  1. **Views, never containers.** BinMatConstView in, BinMatView out, each
///     stride read per row, so an over-aligned matrix (D-4) or a wrapped buffer
///     works unchanged.
...
///     BINCV_ASSERT in debug builds and undefined in release (ARCHITECTURE 5.3).
///
/// ---------------------------------------------------------------------------
/// ALIASING: `dst` MUST SHARE NO WORD WITH `src` -- IN PLACE IS NOT SUPPORTED
///
/// This is the half of D-11 ops/shift.hpp takes, and for the same reason: the
...
// row-geometry and D-11 aliasing vocabulary every kernel under ops/ is written
// in. One copy is the only way the aliasing rule cannot drift.
#include "../impl/kernel_util.hpp"
// impl::maj3, the word primitive T2.7 exists to provide. ARCHITECTURE 7.1's
...
inline void medianRow3(const WordType* above, const WordType* cur, WordType* dst, size_t words,
    ...
    // this kernel the padding invariant (CLAUDE.md; D-13) is carried by the mask
...
/// pixels the median of three IS their majority (ARCHITECTURE 7.1), so each word
