...
/// fill matrix includes both, and "approximately empty" would be a
/// different test.
...
/// default is word granularity, i.e. the MINIMUM stride, which is
...
/// made "the same seed on any word type" false -- and the size matrix
...
/// size,) has its whole tail checked and not merely its trailing partial
...
///, and the reason the container-side check could not cover them.
...
/// @param actual The binCV result, as a view.
...
/// @param file, line Defaulted so the two-argument spelling in the spec
