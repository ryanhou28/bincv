...
**The score** is binCV's own — longest qualifying arc, 9–16, because OpenCV's is
constant on binary content — and cost about 40%. [X-81](EXPERIMENTS.md) recovered most
of that **on x86 only**, by choosing per chunk between a per-corner transpose and eight
nested arc-length masks: never worse, up to **1.42×** better. **The same change was a
10% REGRESSION on the reference device and is not shipped there** — the mask form keeps
sixteen vectors live where X-80's fold consumes them in place, and holding them is what
aarch64's thirty-two registers were buying. **A change that helps one architecture is
not thereby an improvement**, which is why both were measured before either shipped.

### D-68: the covariance was breaking D-6, next door to the kernel D-6 was written for

[X-83](EXPERIMENTS.md) gave `gradientCovariance` a NEON path. It had none:
`3N^2 + N` **scalar** `popcountWord` calls per word — fourteen at `N = 2` — on the one
...
of device `track` outside the iteration loop; a per-stage profile
(`BINCV_LK_STAGE_TIMING`) then put the covariance at **27.5% of `track` and 5.9× slower
than x86**, where the iteration loop — which has a NEON path — is only 3.6× slower.
**That ratio between two stages on the same machine is what named the missing kernel**,
and neither of the two guesses that preceded it (worth 1.9% and 0.0%) would have.

**The 0.0% one was reverted.** Breaking a 248-long dependent `vmla` chain into four
accumulators is a real-sounding optimisation and the compiler was already scheduling
around it. It cost eight registers and bought nothing, so it is not in the code — only
in the log.

**And the second version of the kernel was 1.5× the first**, because the first built its
operand vectors through stack arrays: sixteen stores and four loads a word, each load
waiting on its stores. The shipped one is shuffles of a single vector
`{ax0, ax1, ay0, ay1}` — which **is already** the four diagonal terms, since `a & a` is
`a`. **On aarch64 an array round trip can cost more than the arithmetic it feeds.**

**A correction this leaves on the record:** [D-65](#d-65-one-row-reader-and-the-deployment-target-reaches-248)'s
**2.48×** is stale, not wrong. Measured today under one harness, the device is **3.17×**
against x86's 2.81× — it had not been overtaken, and the claim that it had came from
...
### D-71: the tracker's residual loop is finished at this shape

[X-87](EXPERIMENTS.md) closed E-45 negative — the keypoint batch measured **0.85×** on
NEON, i.e. **slower**, because [D-6](#8-design-decisions) runs in reverse there: aarch64
has `cnt`, so the shipped kernel's four lanes are already spent on the four taps, and
batching keypoints turns one vector operation per plane pair into one per (tap, plane
pair).

**And the loop itself is at its floor for this algorithm.** At `N = 2` the residual needs
`20 N^2 = 80` popcounts of a 31-bit word per row — **2 480 bits** — and the kernel issues
**20 `vcntq_u8` counting 2 560**. **97% of every bit counted is a bit the algorithm asked
for.** On a Cortex-A72 `CNT` is one per cycle, so twenty is a twenty-cycle floor per row
before any AND or accumulate.

**Every optimisation that worked removed operations or memory round trips; every one
aimed at scheduling failed** — four accumulators (0.0%), hoisting the invariant `self`
term (−4%), keypoint batching (0.85×). Going further needs a different algorithm, which
...
### D-75: detection spacing is a plain vectorised loop, and the bit-plane arm lost

A VIO frontend detects to top up, so a fresh corner must be rejected if it lands on a
track already being followed. binCV had no operation for that —
`goodFeaturesToTrack` spaces a detection's corners against **each other**, which is
`cv::goodFeaturesToTrack`'s whole job, and the previous frame's tracks are not among its
inputs and cannot be. The reference supplies the second half separately
(`applyMinDistance`), and **this library's own example had written the loop by hand**,
which is what made the gap a fact rather than an opinion.

**Shipped as `spaceCandidates` in `ops/occupancy.hpp`, API TIER 3** — no `cv::`
equivalent, so no OpenCV name is borrowed. `O(new × live)` float distance tests,
vectorised eight-wide on AVX2 and four-wide on NEON, rejecting on `distance < radius`
strictly, and **allocating nothing**.

**THE BIT-PLANE ALTERNATIVE WAS MEASURED AND IT LOST BY AN ORDER OF MAGNITUDE.** A disc
per live track stamped into a 1-bit frame, one bit test per candidate, is the obvious
binCV-shaped answer — and at the frontend's operating point (640×480, radius 32, 120 live,
300 candidates) it is **26.6× slower on x86 and 7.67× slower on the reference device**,
for 38 400 B against nothing. It does not overtake until ~5 000 candidates on x86 or
~2 000 on aarch64, seven to seventeen times more than a top-up produces
([X-93](EXPERIMENTS.md)).

**The reason is structural rather than an unoptimised loop, and the pre-registration got
it wrong by about fifty.** Stamping touches `πr² = 3 217` pixels to encode what the
exhaustive arm consumes in one distance test per candidate — and that test is vectorised
while the stamp is scalar per row. The estimate counted the disc's word-writes and
omitted both the per-row bound arithmetic (the larger term: 500 ns per disc on x86,
2 427 ns on the device) and the vector width on the other side.

**Both arms ship**, which is the pre-registered rule's clause 2 and not a hedge: the
crossover is measured, so a caller filtering thousands of candidates — masking a response
map before selection rather than a detection's output after it — is on the other side of
it and has a reason to call the mask. The header says which is which, with the numbers.

**The two arms return identical points and that is a test, not a claim.** The disc's row
bounds are found by exact squared comparison **in `float`, in the same operation order
the distance test uses** — not by rounding a `sqrt`, and deliberately not in `double`,
which would be *more* accurate and therefore wrong: the two arms have to round the same
way or a boundary pixel lands on different sides of the same nominal comparison.
Measured: **361 248 pixels against the float predicate, zero disagreements.**

### D-76: the tracker takes an initial-flow guess, and the speed claim is conditional on the motion

`LKParams` used to say there was no initial-flow mode, "since an unused mode is an
untested one". **That was checked against the wrong thing.** `seal_params.yaml` has the
flag off, but the pipeline around it sets `useInitialCorners` from `predictOpticalFlow`
and seeds the tracker from an odometry predictor — IMU-aided tracking, which is what
makes a VIO frontend a VIO frontend rather than a visual one.

**Shipped as `LKParams::useInitialFlow`, and it needed no second buffer.** A point is
written exactly once, at its own entry level, and is untouched at every coarser one — so
`nextPts[p]` still holds whatever the caller put there when the tracker reaches it.
Reading it there *is* reading the guess. That the flag is exactly the identity when the
guess is `prevPts` is pinned by a test rather than argued: 35 points, 0 differ in
position, `status` or `err`.

**THE SPEED CLAIM IS CONDITIONAL AND THE HEADER MAY ONLY MAKE IT CONDITIONALLY**
([X-94](EXPERIMENTS.md)). At **5 px** of motion a guess is worth 0.86–1.13× — nothing —
because the coarsest level of a 1/2/2/2 ladder already starts 0.62 px from the answer;
and a guess wrong by more than the true motion **costs about 10%**. At **24 px** it is
worth 3.70× on x86 and 5.22× on the device, and a guess wrong by 8 px still gives
3.5×/4.1×. Seed it from a prediction trusted to within the frame's own motion, or not at
all.

**The large-motion figure is mostly the cost of a failure, not the saving from a guess,
and D-53 requires saying so.** Without a guess at 24 px the tracker returns a mean
endpoint error of **54.7 px while reporting `status = 1` for every point**. The guess
routes around that; it does not fix it, and a caller without an IMU still has it. That is
[E-48](#9-open-questions-and-planned-experiments), registered separately so this decision
cannot be read as having closed it.

### D-77: the residual reject ships default-off, because its threshold is a property of the content

[X-94](EXPERIMENTS.md) found the tracker returning 54.7 px of endpoint error while
reporting `status = 1` for every point: past the pyramid's reach it does not fail, it
answers confidently, and `minEigThreshold` does not catch it because the windows are
well conditioned — they are matched to the wrong place.

**`LKParams::maxResidual` clears `status` when the level-0 residual exceeds it**, and the
residual is the number `err` has always carried. The bidirectional consistency check
E-48 also registered — two more planes per pyramid level — **was never built**, because
the free arm works: at 0.05 on synthetic translation it keeps 816 good points and rejects
all 816 lost ones, through both the scalar and the AVX2 batch paths.

**THE DEFAULT IS OFF AND THAT IS THE RESULT, NOT A FAILURE TO CHOOSE.** The same 0.05
that costs nothing on synthetic texture takes V1_02's median track lifetime **from 79
frames to 1** and runs the detector on all 79 frames. `err` is a mean absolute difference
over the window, so it measures how well *this* content matches under *this*
binarisation: real edge maps sit four to five times higher than synthetic texture
(median ≈ 0.105 against 0.019–0.029). **A threshold calibrated on one imagery is not
conservative on another, it is destructive** — so binCV ships the mechanism and the
calibration procedure, and refuses to ship a number it can only have measured on one
sequence. [D-53](#d-53-a-ceiling-prices-the-operation-not-the-algorithm) is why: a rule
that removes the failures by also removing the tracks is not a fix.

**It also is not free, and the pre-registration said it would be.** Switching it on makes
the residual computed for a caller who passed `err == nullptr`: **2.16× on x86, 1.86× on
the reference device.** The comparison is free; the window pass is not. The same
measurement prices something that was already shipping and had never been quantified —
...
### D-79: the tracker's stack cost is declared in bytes and checked at compile time

The tracker stages each window into two stack buffers so the iteration loop reads a
compact copy instead of re-walking the frame. **That is memory bought for speed**, and
`kStagedMaxRows = 64` bounds it — but it bounds the **shape**, and lets the **size**
float with the bit depth: `64 × (7N + 2)` words plus 24 bytes of scalars.

| | 32-bit words | 64-bit words |
|---|---|---|
| N = 1 | 2 328 B | 4 632 B |
| N = 2 (shipped) | **4 120 B** | 8 216 B |
| N = 8 | **14 872 B** | 29 720 B |

**On a desktop's 8 MB stack this is noise. On a Cortex-M with a 16 KB stack the N = 8
figure is the whole stack, and overflowing one there is not a crash — it is silent
memory corruption.** That asymmetry is the whole decision: the same number is
irrelevant on one target and fatal on another, so it cannot be left implicit.

**`BINCV_STAGING_BUDGET_BYTES` declares it and `static_assert` checks it**, at the point
where the buffers are instantiated — so it fires only for the `(N, WordType)` a caller
actually uses. An embedded build of the shipped 1-bit ladder never trips it; one at
N = 8 gets a build error naming the number. `bincv::stagingStackBytes<N, W>()` is public
so an integrator can size a thread stack, or assert against a budget binCV cannot know.

**A run-time byte CAP was the other candidate and was declined.** Capping rows to fit a
budget would send a normal 31-row window to the slower unstaged path at high `N` (about
28 rows fit in 4 KB) — trading a silent overflow for a silent slowdown, which is not an
improvement. **A build error is the only outcome a person is forced to look at**, and
this is a decision that should be made by a person, once, at integration time.

The default admits everything binCV ships, so nothing changes for existing callers; the
value is that the cost is now stated, checked, and queryable instead of being a comment.

### D-80: 64-bit callers get the vector kernels by narrowing, not by a second implementation

**Asked by the owner**, and it is a wider question than E-29 recorded: five kernels are
gated on `sizeof(WordType) == 4` — the LK residual, the covariance lane kernel,
`unpackWord32`, `edgeThreshold` and `packQuant` — so a caller holding 64-bit words takes
a scalar fallback in the tracker *and* in the sensor stage.

**Narrowing is a complete substitute for a native 64-bit kernel, and the measurement is
close to a tautology once stated.** On little-endian a 64-bit plane already IS a 32-bit
plane with twice the stride ([D-73](#d-73-64-bit-callers-get-the-vector-tracker)), so
running the 32-bit kernel over a narrowed view is the same instructions over the same
bytes. Measured on `edgeThreshold`: **1.00× of native on the reference device**, 0.96× on
x86, and **0 of 307 200 pixels differ** ([X-97](EXPERIMENTS.md)).

**So a native 64-bit kernel could at best MATCH it, and would cost a second
implementation of every gated kernel** — plus the standing correctness liability that
[E-25](#9-open-questions-and-planned-experiments) made this project pay for elsewhere.
It is not written.

**THE BASELINE IS WHAT MAKES THIS OBVIOUS, AND IT WAS FIXED BEFORE MEASURING.** Against
the scalar fallback a native kernel looks like a 32× win. Against narrowing it looks like
nothing. Both numbers are true; only the second answers the question, and X-97's rule
named it in advance for exactly that reason.

**What ships is `narrowPlaneMutable`, and guidance.** The const `narrowPlane` covers
reading, which is all the tracker needs; the sensor-stage kernels WRITE, so they needed
a mutable spelling. `uint32_t` remains the default (D-14): the 64-bit stride penalty is
**+33% at the upper pyramid levels**, re-measured here, and those are the levels an
embedded target is tightest on. A caller who genuinely wants 64-bit storage keeps it and
narrows at the call.
...
| ~~**E-31**~~ **RESOLVED — NOT APPLICABLE TO A HEADER-ONLY LIBRARY** | Should binCV get runtime POPCNT dispatch on x86, or simply require the instruction? | [X-57](EXPERIMENTS.md) measured the software fallback costing **3.75×**; `BINCV_X86_POPCNT` now defaults ON, so the question is whether the minimum can be removed. | **[X-60](EXPERIMENTS.md) measured the obvious mechanism and it does not work.** `__attribute__((target(...)))` **blocks inlining**, turning an inline hot function into 310 calls per window and costing 1.9×. So dispatch for `popcountWord` cannot be per-call; it would have to be per-**kernel**, chosen once at a coarse boundary — or the 2008-era minimum simply kept, which is what several vision libraries do. | **Answered: compile-time detection is already what happens, and it is the right answer.** Runtime dispatch buys something only for a PRE-BUILT binary shipped to unknown CPUs; binCV is header-only, so the consumer always compiles and always knows their target. [X-60](EXPERIMENTS.md) had already priced the only mechanism at **1.9× worse than the 3.75× problem**. What remains is the [F-5](EXPERIMENTS.md) shape — an include-only build silently takes the fallback — and that is handled by `simdStatus()` reporting `__POPCNT__` ([D-78](#d-78-a-fast-path-may-not-depend-on-the-build-system-when-the-compiler-already-knows)), not by dispatch. | D-47 | **Phase 5** |
...
| ~~**E-38**~~ **RESOLVED — DECLARED AND CHECKED, NOT CAPPED** | `StagedWindow` and `TapCache` cap at **64 rows**, so their stack cost scales with `N`: 4 KB at `N = 2`, **≈15 KB at `N = 8`**. Should the cap be on BYTES instead? | [X-70](EXPERIMENTS.md) shipped both and stated the figure rather than hiding it. A row cap is the wrong axis — it fixes the shape, not the footprint, and footprint is the constraint [CLAUDE.md](../CLAUDE.md) says wins ties. | **A byte cap would decline the shipped 31-row window at `N = 8`** (≈28 rows fit in 4 KB), sending it to the unstaged path — a correctness-preserving but silent slowdown at high `N`. The shipped ladder is `1/2/2/2` so nothing today is affected, which is exactly why this is registered rather than guessed: **there is no measurement of what high-`N` callers exist.** | **Answered: neither a row cap nor a byte cap — a compile-time BUDGET.** A run-time byte cap would send a normal 31-row window to the slower unstaged path at high `N`, trading a silent overflow for a silent slowdown. `BINCV_STAGING_BUDGET_BYTES` + `static_assert` makes it a build error instead, checked only for the `(N, WordType)` a caller instantiates. Measured: **4 120 B at the shipped N = 2, 14 872 B at N = 8**, double at 64-bit words. [D-79](#d-79-the-trackers-stack-cost-is-declared-in-bytes-and-checked-at-compile-time) | D-61 | **Phase 5** |
...
| ~~**E-48**~~ **RESOLVED — MECHANISM SHIPS, DEFAULT OFF** | The tracker fails SILENTLY at large motion: on a 24 px translation it returns a mean endpoint error of **54.7 px — nearly twice the true displacement — while reporting `status = 1` for all 816 points**, at 12.08 iterations per point-level. | Found by [X-94](EXPERIMENTS.md) while measuring something else. Same family as [X-20](EXPERIMENTS.md)'s monotone degradation with 1-bit pyramid levels and its level-0 stationary point; what is new is the magnitude and that no loss rule catches it. An initial-flow guess routes around it — **it does not fix it**, and a caller without an IMU still has it. | Two candidate arms, both with a real cost: a **bidirectional consistency check**, which needs the NEXT frame's derivative — two more planes per level, the footprint trade `LKLevel` currently declines — and a **residual-magnitude reject at convergence**, nearly free but a new loss rule, so [D-53](#8-design-decisions) requires it be measured on TRACK LIFETIME rather than on time. | **Answered: the free arm works and the bidirectional check was never needed. But the threshold does not travel and the reject is not free.** At 0.05 it removes 100% of the failures and 0% of good tracks on synthetic content — and on V1_02 the SAME value takes median track lifetime from 79 frames to 1. `err` is content-dependent. And it costs **2.16× on x86 / 1.86× on the device**, because switching it on computes a residual an `err == nullptr` caller was not paying — the pre-registration predicted 'unmeasurable' and was wrong. Ships DEFAULT-OFF with a calibration procedure. [X-95](EXPERIMENTS.md), [D-77](#d-77-the-residual-reject-ships-default-off-because-its-threshold-is-a-property-of-the-content) | D-53 | **unscheduled** |
| ~~**E-46**~~ **RESOLVED — THE MASK LOST** | A VIO frontend must space NEW detections against the tracks it is already following. binCV does not: its spacing filter spaces new corners only against each other, and `examples/vio_frontend.cpp` hand-rolls the missing loop. Which shape ships? | The reference calls `applyMinDistance(corners, prevCorners, maskRadius)` on **every detect**; without it a top-up plants a fresh feature on top of a point already being tracked. Reported from outside the project, and our own example is the evidence. | Two arms: **exhaustive** `O(new × live)` float distance tests, costing no memory; and a **1-bit occupancy mask** — stamp a disc per live track, one bit test per candidate — costing one 1-bit frame (38 400 B at 640×480). **Bit-parallel is a reason to MEASURE the mask, not a reason to assume it wins**; a first estimate puts the two at the same order at the frontend's operating point. | **Answered: the plain loop, and not narrowly. Exhaustive is 26.6× faster on x86 and 7.67× on the device at the frontend's operating point, for 0 B against 38 400 B; the mask does not overtake until ~5 000 candidates (x86) or ~2 000 (aarch64). The pre-registered crossover estimate of ~100 was wrong by ~50×, because it counted the disc's word-writes and neither the per-row bound arithmetic nor the vector width on the other side.** [X-93](EXPERIMENTS.md), [D-75](#d-75-detection-spacing-is-a-plain-vectorised-loop-and-the-bit-plane-arm-lost) | — | **Phase 5** |
| ~~**E-47**~~ **RESOLVED — SHIPPED, CLAIM CONDITIONAL** | Does an initial-flow guess actually cut tracking time, and past what guess error does it stop helping? | `LKParams` declined the mode because "an unused mode is an untested one" — **checked against the wrong thing**: `seal_params.yaml` has the flag off, but HybVIO's `computeOpticalFlow` turns it on from `predictOpticalFlow` and seeds LK from an odometry predictor. IMU-aided tracking is the normal VIO case. | The feature ships on its own merits; what needs a rule is the **claim**. Iterations per point-level and `track` wall-clock, for a perfect guess and for guesses corrupted by 0.25/0.5/1/2/4 px, both architectures, through the existing `BINCV_LK_ITERATION_HISTOGRAM` hook. | **Answered: only conditionally. At 5 px of motion a guess is worth 0.86–1.13× — nothing — because the coarsest level already starts 0.62 px out. At 24 px it is worth 3.70× (x86) and 5.22× (device), and a guess wrong by 8 px still gives 3.5×/4.1×. A guess wrong by MORE than the true motion costs ~10%.** [X-94](EXPERIMENTS.md), [D-76](#d-76-the-tracker-takes-an-initial-flow-guess-and-the-speed-claim-is-conditional-on-the-motion) | D-58 | **Phase 5** |
| ~~**E-40**~~ **RESOLVED — THE OPERATION LEFT THE PATH** | `fromCVMat` is **33% of the x86 frontend at one thread and 53% at twelve**, and does not scale. Optimise it on **both** architectures. | [X-67](EXPERIMENTS.md) found it at 20.4%; [X-69](EXPERIMENTS.md)/[X-70](EXPERIMENTS.md) shrank `track` around it until it became the largest single item. The 1-bit path is a **per-pixel branch and read-modify-write**. | **Bit-plane extraction from bytes is what a move-mask instruction does.** `_mm256_movemask_epi8` returns one plane of 32 pixels in one instruction; aarch64 has no movemask but AND-with-bit-weights plus pairwise-add gives 16 in about six. A portable **branchless** arm is measured too, because if it gets most of the win the intrinsics are not worth their maintenance. | **Answered by removal rather than by optimisation.** T5.8 / [X-89](EXPERIMENTS.md) gave binCV its own sensor stage, and its edge filter writes bit-planes directly — so `fromCVMat` is **no longer called by the frontend at all**. The 33% it was registered about does not exist to optimise. What replaced it is the sensor stage, measured at 9.8% of the frontend and already vectorised on both architectures. | D-59, D-62 | **Phase 5** |
| ~~**E-45**~~ **RESOLVED — NEGATIVE** | *(premise corrected by [X-82](EXPERIMENTS.md): the device is **3.17×** against x86's 2.81× and was never overtaken; D-65's 2.48× is stale.)* [D-66](#d-66-eight-keypoints-per-avx2-register-and-the-refill-is-the-design)'s keypoint batch is **x86 only** — `#if defined(BINCV_X86_LK_BATCH)`. aarch64 got nothing from it, and x86 has now OVERTAKEN the device on the frontend ratio (**2.81× against 2.48×**). Should the batch be ported to NEON? | The device was the faster side of this project for its whole history and is no longer. That is a reason to look, not a reason to port. | **THE PREDICTION, WRITTEN BEFORE MEASURING: IT WILL NOT TRANSFER, AND [D-6](#8-design-decisions) IS WHY — IN REVERSE.** x86 needed keypoint batching because **AVX2 has no popcount at all**: the nibble-table emulation costs six operations for thirty-two bytes and only pays off spread across eight keypoints. **aarch64 HAS a vector popcount**, and `slicedSignedSum`'s NEON path already fills all four lanes with the four plane pairs (X-33) and already carries accumulators across the window (X-40). Batching four keypoints would need one register per plane pair instead of one register for all four — **the same lane-work, rearranged**. So the expected win is ~1×. **This is an argument and not a measurement**, which is exactly why it is registered rather than acted on; the decisive number is a device-side ceiling in X-66's shape, and if it clears 1.5× the prediction is wrong and the port is worth writing. | **Answered: it does not transfer, and the margin is bigger than "no win" — [X-87](EXPERIMENTS.md) measured the ceiling at 0.85×, i.e. the batched form is 1.18× SLOWER.** D-6 in reverse, exactly as predicted: aarch64 has `cnt` and the shipped kernel already fills all four lanes with the four TAPS, so one `vand`+`cnt`+byte-add covers four taps against one plane pair; batching keypoints turns that into one (tap, plane pair) per vector — four times the operations — and also loses the magnitude broadcast. **Band D, E-45 closes negative, the port is not written.** | Whether the deployment target has a tracking lever left, or whether D-6 closes this one too. **[X-82](EXPERIMENTS.md) deliberately did NOT measure the ceiling: its Part A decomposition found the covariance at 27.5% of device `track` with a known cause and a known fix ([D-68](#d-68-the-covariance-was-breaking-d-6-next-door-to-the-kernel-d-6-was-written-for)), which was worth doing first. The prediction against the port is still only an argument. | D-6, D-66, D-68 | **Phase 5** (X-87) ✔ |
...
| ~~**E-44**~~ **RESOLVED — SHIPPED** | The bit-plane FAST's **score costs ~40% of the operation** — 1.39× with it, **1.75×** without. Can it be computed bit-sliced instead of per corner? | [X-80](EXPERIMENTS.md) measured the split. OpenCV's own score is **the same number for every corner** on binary content, so binCV reports the longest qualifying arc (9–16) instead — which needs a 16-word bit transpose per corner, and corners are 2% of pixels on a real binarised frame. | **The arc length is already half-computed.** The detection tree yields "a run of 9 starts here" as a mask; seven more doubling passes would yield runs of 10 to 16, and the score is then a bit-sliced sum of eight masks — per CHUNK rather than per corner. **It is not obviously cheaper**: 224 extra vector operations per chunk unconditionally, against ~78 scalar per corner, so it wins only above roughly 3% corner density. **Measure both; do not assume the bit-sliced form wins because it is bit-sliced.** | **Answered, and the registered caution was the right one: NEITHER ARM WINS.** [X-81](EXPERIMENTS.md) measured both across a density sweep and they **cross at about 1% corner density**, each losing by up to 1.4× on the wrong side. The prediction here said 3%; the shape was right and the number pessimistic. **Shipped as a per-CHUNK choice on AVX2** — never worse than the better fixed arm, **up to 1.42×** better than X-80's path, and only ~3% on the frontend's own edge map because corners there are CLUSTERED and most chunks fall below the threshold anyway. **NOT shipped on NEON: the same change cost the reference device 2.36× → 2.10×**, and the threshold sweep showed the loss even where the mask path is never taken, so it was the restructuring — the mask form keeps sixteen vectors live across eight passes where X-80's fold consumes them in place, which is what aarch64's thirty-two registers were being spent on. Output identical across all three arms over 2 100 corners. | Whether the 1.75× detection number is reachable with the score still present. **Partly, on one architecture — and a cross-architecture win is not a win.** | D-67 | **Phase 5** (X-81) ✔ |
| ~~**E-42**~~ **CLOSED — IT IS THE CALLER'S PARAMETER** | `seal_params.yaml` caps LK at **20 iterations**, and [X-78](EXPERIMENTS.md) measured that **3.6% of point-levels run the whole cap and converge to nothing — burning 22% of every iteration the tracker executes.** Should the cap come down? | X-78 counted iterations directly for the first time: **72.6% of point-levels finish in two or fewer**, and the distribution is bimodal with a tail pegged at the cap. The tracker spends a fifth of its time on points it is about to drop. | **It is an ACCURACY change, not a free one**, which is why it is registered rather than taken. A point at iteration 15 has not converged but its estimate is not therefore worthless, and `status` does not currently distinguish "converged" from "ran out". The arms are the cap itself (20 / 10 / 5) measured against **track lifetime and endpoint error**, not against time alone — [D-53](#8-design-decisions) applies, and a cap that speeds up `track` by shortening tracks has not helped anyone. | **Closed by the owner's call: keep the reference's value.** `maxIterations` is already a caller parameter and binCV's default of 20 is `seal_params.yaml`'s — tighter than `cv::calcOpticalFlowPyrLK`'s own 30. Changing the DEFAULT would silently re-base every recorded number and is an accuracy change, not a free one. X-78's finding stands as documentation so a caller who wants 10 can choose it knowingly. | D-58 | **Phase 5** |
| **E-37** → **[#8](https://github.com/ryanhou28/bincv/issues/8)** | `loadLevel0` converts **both** frames every frame and `build()` builds **both** pyramids — but this frame's `next` is next frame's `prev`. Should the frontend ping-pong? | [X-67](EXPERIMENTS.md) measured `fromCVMat` at **20.4% of the frontend at T=1 and 34.4% at T=4**, and roughly half of it is recomputation. | **Not fixed where it was found, on purpose.** Changing the harness changes every recorded frontend number in [EXPERIMENTS.md](EXPERIMENTS.md) — that is a decision about the denominator, not a cleanup, and it needs its own rule and a restated baseline. Note OpenCV's `calcOpticalFlowPyrLK` also rebuilds both pyramids per call, so the redundancy may be **symmetric** and removing it only on binCV's side would flatter binCV. **Measure both before changing either.** | Whether ~10% of the x86 frontend is recomputation, and whether the denominator moves with it. | D-59 | **Phase 5** |
...
| ~~**E-25**~~ **RESOLVED — KEEP BOTH** | The hand-written `pyrDown` is now only **1.19×** faster than the generic route computing the same function. Should it be deleted, leaving one implementation for all six filters? | [X-42](EXPERIMENTS.md) closed the gap from 2.96×. Two implementations of `BOX_2x2` is a standing correctness liability that `tests/test_pyramid.cpp` currently pays for by holding them to agreement. | **Not a free call.** The hand-written route is what **every prior result in this project was measured on**, including D-35's criterion-4 numbers, so deleting it re-bases the whole speed record by 1.19% — small, but it must be re-measured rather than assumed. Against that: one implementation, one place for the next optimisation, and the three structural costs (serial accumulation, materialised intermediate, worst-case widths) become worth attacking because they would then be on the shipped path. | **Answered: two, by the owner's call.** The framing "delete the faster one" was misleading and is corrected here: the GENERIC route is needed regardless, because it serves all six filters; the hand-written one is a special case for `Box2x2` alone. So the choice was one code path or two, not fast or slow. Deleting the special case would cost **1.19× on a stage that is 9.7% of the frontend — about 1.5% of frontend time** — and re-base every recorded number. Both ship; `tests/test_pyramid.cpp` continues to hold them to agreement, which is the price of the second path and is paid deliberately. | D-39 | **Phase 5** |
...
| ~~**E-29**~~ **RESOLVED — NARROWING, NOT A SECOND KERNEL SET** | Should `residualSums` get a `uint64_t` NEON path, so the word type is genuinely free? | [X-54](EXPERIMENTS.md) measured the cost of not having one: `uint64_t` loses 1.32× on track while winning 1.66× on build. The guards are a **specialisation gap, not an ISA property** — aarch64's `CNT` counts a 128-bit register regardless of how the caller sliced it. | Same shape as the x86 gap ([X-52](EXPERIMENTS.md)): one kernel, one missing specialisation. Worth it only if a build-dominated pipeline appears, or if the `uint64_t` build win (1.66×) is wanted without the track loss. | **Answered: real, via views rather than via duplicate kernels.** Narrowing runs a 64-bit buffer through the 32-bit vector arm at **1.00× of native on aarch64** and 0.96× on x86, bit-identical over 307 200 pixels — it is the same kernel on the same bytes, so a native 64-bit kernel could at best MATCH it. The question was also broader than this row: five kernels are gated, including `edgeThreshold` and `packQuant`, where the fallback costs **32× (x86) / 8.3× (device)**, not X-54's 1.32×. [X-97](EXPERIMENTS.md), [D-80](#d-80-64-bit-callers-get-the-vector-kernels-by-narrowing-not-by-a-second-implementation) | D-45 | unscheduled |
