...
The trail behind every number in [cuda.md](cuda.md): how each one was earned, the
layouts that were tried, the `ncu` readings, the arms kept and the arms dropped, the
claims withdrawn and why, and the rows that are not results.

**This file is not optional and it is not read top to bottom.** The results document
states what the backend costs and saves; this one is the reason those figures are worth
quoting rather than believing, so every role-bar row over there links into the section
here that earned it. Read the section you are about to quote from.

How a *speed difference* is decided — the paired design, the three verdicts, the worked
examples — is a third document: [methodology-timing.md](methodology-timing.md). How
*memory* is measured is [methodology-memory.md](methodology-memory.md).

Tables here follow the same shape as the results document: both sides' measured values
with the unit in the column header, the baseline first, the better side in bold, speed and
memory never in one column, and a ratio only where it adds something the two values do not
already say. Where a ratio does appear, the cell names the winner in words, so a loss reads
as a loss with no marker and no legend.

| section | what is in it |
|---|---|
| [Dense disparity, both entries](#dense-disparity-both-entries) | the headline path, the census layout that closed its gap, the warp-cooperative box matcher and the model it refuted |
| [Foundation ops](#foundation-ops) | the microbenchmarks, and the covariance signature the 467× came from |
| [The sensor stage](#the-sensor-stage-threshold-binarize-edgethreshold) | `threshold`, `binarize`, `edgeThreshold`, and the 37.8–49.9× that was withdrawn |
| [The window family](#the-window-family-morphology-and-the-medians) | morphology and the medians, including the 123× that is the competitor's design rather than binCV's |
| [The pyramid, the resident ladder and shift](#the-pyramid-the-resident-ladder-and-shift) | the tie that was written down as a pass first, and the 5.61× withdrawn with it |
| [The frontend on device](#the-frontend-on-device) | the 1.22×-slower negative and its closure, the stage split, the one kernel the profiler located, FAST |
| [Where binCV has no structural advantage](#where-bincv-has-no-structural-advantage-said-plainly) | the corner response, sub-pixel refinement, the derivative |
| [Arms measured and left off](#arms-measured-and-left-off) | optimized arms that lost their timed case and ship off by default |
| [The harness limit that decided more than it should](#the-harness-limit-that-decided-more-than-it-should--resolved-and-what-it-did-not-change) | the range test, its replacement, and the three effects that did not become wins |
| [Tracking on device](#tracking-on-device) | Lucas-Kanade end to end, the density at which the lead stops, the 1.56× kernel loss inside a wall-clock win |
| [Sparse stereo, matching, and the geometry](#sparse-stereo-matching-and-the-geometry) | descriptor matching, the gated matcher, block matching, and the RANSAC negative entire |
| [The profiler pass](#the-profiler-pass) | enablement, 27 kernels, the scale sweep, traffic, what it confirmed and what it contradicted, R1–R10 |
| [Harness defects found while measuring](#harness-defects-found-while-measuring) | four defects that were producing published numbers, and where each is recorded |
| [How the numbers were earned](#how-the-numbers-were-earned) | the optimization ladder per matcher, and the recorded negatives along it |
| [What is not measured](#what-is-not-measured) | the platforms, the tools that do not work here, and the things named, measured and not built |
...
| arm | clock | time (ms) | against its reference arm |
|---|---|---|---|
| binary, reference — the baseline for the rows above it | kernel | 1.71 | — |
| **binary, word-parallel** (shipped) | kernel | **0.069** | 25× over reference |
| binary, per-pixel sliding | kernel | 0.408 | 4.2× over reference |
| binary, upload + kernel + download | e2e | **0.28** | — |
| binary, host CPU arm (same machine) | cpu | ~19 (17% spread) | — |
| census transform, packed (both frames) | kernel | 0.14 | — |
| **census matcher (K=24), warp-cooperative box** (shipped) | kernel | **0.369** | 2.49× over the packed arm |
| census matcher (K=24), packed | kernel | 0.94 | 44× over reference |
| census matcher (K=24), plane layout | kernel | 7.78 | 5.3× over reference |
| census, host CPU path (same machine) | cpu | ~309 (5% spread) | — |
...
`cudaMemGetInfo` reading [the role bars](cuda.md#the-role-bars) use.
...
previous round. (The 1.38× and 1.43× these reports have carried at different
points are the same row on earlier sweeps; its run-to-run scatter is 1.14×, so
the three readings are one number, not three.) It is **larger** than StereoBM on the memory meter; that is in
[cuda.md's headline](cuda.md#the-headline) and is not softened here.
...
ms, three runs). The 8.55× previously quoted here for that gap was the
...
| op | GPU kernel (ms) | host CPU arm (ms) | note |
|---|---|---|---|
| upload packed frame (45 KB) | 0.009 | — | contiguous fast path |
| upload wide frame (361 KB) | 0.048 | — | |
| download wide frame (361 KB) | 0.080 | — | |
| `bitwiseAnd` | 0.007 | **0.002** | **the "memcpy-bound" label this row carried is withdrawn** — see [the scale sweep](#the-scale-sweep-which-decides-both-asserted-assumptions) |
| `countNonZero` | 0.014 | **0.004** | |
| `packBits` (sensor stage) | **0.010** | 0.025 | `__ballot_sync` packer |
| `packQuant` N=2 (N-bit ingestion) | **0.015** | 0.064 | N ballots per 32 pixels |
| `censusTransform` (24 planes) | **0.073** | 2.02 | shared-memory tile, 17.1× over its own reference |
...
| arm | time (ms) | what the row says |
|---|---|---|
| per-window loop — 200 launches of the single-region form | 3.667 | **the batch is 467× faster** |
| host `countCovariance` ×200 | 0.015 | the batch is 1.9× the CPU |
| **`countCovarianceBatchAsync`** — one launch | **0.008** | |
...
| op | geometry | cv::cuda (ms) | binCV (ms) | speed | device memory, 752×480 |
|---|---|---|---|---|---|
| `threshold` → bits | 752×480 | 0.0100 | **0.0094** | **parity, 1.00×** | binCV **2.46×** smaller |
| `threshold` → bits | 3840×2160 | 0.0367 | **0.0272** | binCV **1.34×** faster | not measured |
| `edgeThreshold` | 752×480 | 0.0821 | **0.0096** | binCV **8.1×** faster | binCV **24.6×** smaller |
| `edgeThreshold` | 1920×1080 | 0.1886 | **0.0164** | binCV **11.1×** faster | not measured |

Every memory figure in these op-expansion tables is taken at 752×480,
...
post-fix reading from `cuda_sensor_benchmark`.

**The role-bars table does not carry the same numbers, and a sentence here used to
say that it does.** [cuda.md's role bars](cuda.md#the-role-bars) read 0.0091 ms
against 0.0084 at 752×480 and 0.0362 against 0.0268 at 3840×2160, where this table
reads 0.0100 against 0.0094 and 0.0367 against 0.0272 — and the two tables state the
same comparison in opposite directions, 1.34× here against 0.743 there. Same
operation, two harnesses, two sweeps, and one of them — `cuda_sensor_benchmark` —
had the scoping defect recorded in
[methodology-timing.md](methodology-timing.md#what-changed-when-the-rule-was-applied).
Which pair a reader should quote is **not settled here**; it is reported, and the
claim that they were one protocol is withdrawn.
...
| case | geometry | cv::cuda (ms) | binCV (ms) | speed | device memory, 752×480 |
|---|---|---|---|---|---|
| `erode` rect 3×3 | 752×480 | 0.1429 | **0.0118** | binCV **11.6–11.9×** faster | binCV **16.0×** smaller |
| `erode` rect 3×3 | 1920×1080 | 0.2011 | **0.0148** | binCV **13.0–13.5×** faster | not measured |
| `morphologyEx` OPEN 3×3 | 1920×1080 | 0.4209 | **0.0238** | binCV **15.1–16.1×** faster | not measured |
| `erode` ellipse 5×5 | 752×480 | 0.2196 | **0.0154** | binCV **12.8–13.9×** faster | (as rect 3×3) |
| `erode` ellipse 5×5 | 1920×1080 | 0.4048 | **0.0176** | binCV **22.9×** faster | not measured |
| `medianWide` K=9 | 752×480 | 6.1888 | **0.0249** | binCV **235–249×** faster | binCV **123×** smaller |
| `medianWide` K=9 | 1920×1080 | 34.1522 | **0.0538** | binCV **634–740×** faster | not measured |
| `denoiseMedian3` vs binCV's own byte arm | 4096×2160 | 0.0388 | **0.0087** | binCV **4.6–5.0×** faster | binCV **8.58×** smaller |
...
| comparison | geometry | cv::cuda (ms) | binCV (ms) | speed | device memory |
|---|---|---|---|---|---|
| `buildPyramidBox` vs `resize` INTER_AREA ×3 | 752×480 | **0.0233** | 0.0240 | **a tie, 1.02×** | binCV **7.17×** smaller |
| `buildPyramidBox` vs `pyrDown` ×3 | 752×480 | 0.0256 | **0.0239** | a tie, 0.95× | (same ladder) |
| `shift` vs `cudaMemcpy2DAsync` | 752×480 | 0.0092 | **0.0091** | **a wash** | binCV **7.8333×** smaller (formula) |
...
eight-unit rule [cuda.md](cuda.md#conditions-you-cannot-read-a-number-without) quotes — but clearing it is not the same as
...
| edge | corners | nextPow2 | cv::cuda FAST (ms) | binCV (ms) | binCV against cv::cuda | disjoint | set gate |
|---|---|---|---|---|---|---|---|
| 10 | 23,274 | 32,768 | **0.1286** | 2.2726 | **17.9× slower** | 7/7 | agree 7/7 |
| 17 | 19,898 | 32,768 | **0.1338** | 2.2563 | **16.8× slower** | 7/7 | agree 7/7 |
| 30 | 12,379 | 16,384 | **0.1222** | 1.0643 | 8.8× slower | 7/7 | agree 7/7 |
| 50 | 886 | 1,024 | 0.1063 | **0.0532** | **2.1× faster** | 6/7 | agree 7/7 |
| 80 | 161 | 256 | 0.1004 | **0.0353** | **3.8× faster** | 7/7 | agree 7/7 |
...
Earlier rounds of these reports recorded an open question: three effects had
...
[methodology-timing.md](methodology-timing.md)), and separation is printed beside
every verdict as a fact.
...
would move every number in these reports — a change of its own, not a correction
to this one.
...
| cadence | HOST binCV (ms/frame) | DEVICE binCV (ms/frame) | which is faster | ranges |
|---|---|---|---|---|
| re-detect every 10 frames | 1.932 [1.779–1.992] | **0.424** [0.383–0.471] | the device, by **4.56×** | disjoint 7/7 **and globally** |
| re-detect every frame | 6.455 [6.388–6.652] | **1.056** [0.974–1.159] | the device, by **6.11×** | disjoint 7/7 **and globally** |
...
anywhere in these reports, so the rows below are judged by the project's rule
...
that separates those two statements is recorded in
...
**G3 fails by at least 15.9× — the median is 18.58× — and this is the result.** 7 independent process runs, each
...
1. **No duration here is a timing number.** `ncu` serializes, replays and locks
   ...
   profile*. Every speed number in these reports comes from a free-running
...
memcpy-bound, and the claim about the *operation* is right. But the foundation
table above attached it to the **752×480** row, and there the kernel reads **6.9% of peak
...
Two of these change what these reports should say about memory.
...
## Harness defects found while measuring

Four defects in the measuring apparatus were found by using it, and all four were
producing published numbers or published counts. They are gathered here because a reader
assessing how much to trust this backend's figures should be able to find them in one
place; each is recorded in full where it was found.

| defect | what it was doing | where it is recorded |
|---|---|---|
| `BINCV_CHECK_EQ` evaluated its first argument twice | 2,112 of a published 38,901 check count never existed as distinct assertions, and `BINCV_CHECK_EQ(cudaFree(p), cudaSuccess)` was a double free | [cuda.md, Coverage](cuda.md#coverage) |
| the dense-disparity test pair was an exact shift | a deliberately broken halo passed 508 checks byte-identically, because at an exact shift the argmin is insensitive to window-sum errors of a few hundred | [the warp-cooperative box matcher](#the-warp-cooperative-box-matcher-and-the-model-it-refuted) |
| `gftt_wall` and the frontend wall-clock pair assembled their paired summary by hand | two published role bars printed a 0–0 sign split and p = 1 over rounds that are in fact 105–0, and took the median from the arithmetic even-count midpoint | [methodology-timing.md](methodology-timing.md#what-changed-when-the-rule-was-applied) |
| `cuda_sensor_benchmark` never set its per-geometry scope | its two geometries pooled under one key, so the aggregation read the difference *between the geometries* as run-to-run scatter | [methodology-timing.md](methodology-timing.md#what-changed-when-the-rule-was-applied) |

Only the first two moved anything a reader would have quoted: the check count, and a test
that could not have caught the bug it was written for. The third changed no published
ratio — it changed what the harness could say about one — and the fourth was reading its
own sweep as noise on a geometry that carries no bar.
...
Kernel-resident, 752×480, in milliseconds; the shipped arm of each column is bold.

| stage | binary matcher (ms) | census matcher (ms) | census transform (ms) |
|---|---|---|---|
| reference kernel (one thread per pixel) | 1.71 | 39.3 | 1.23 |
| shared-memory tiling, 8-wide disparity tiles | 0.58 | 11.7 | — |
| sliding vertical window, 16-row strips | 0.408 | 7.75 | — |
| packed descriptor layout | — | 0.94 | — |
| word-parallel bit-slicing, one thread per word | **0.069** | — | — |
| warp-cooperative separable box | — | **0.369** | — |
| shared-memory tile + all-K ballots | — | — | **0.071** |
