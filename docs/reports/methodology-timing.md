...
`backends/cuda/benchmark/paired_stats.hpp`. It is therefore the rule behind the speed
figures in every report in this directory, not only the CUDA ones; the worked examples
below are drawn from [cuda.md](cuda.md) and [cuda-evidence.md](cuda-evidence.md) because
that is where it was written down and tested.

| section | what it settles |
|---|---|
| [How a difference is decided](#how-a-difference-is-decided) | the paired design, and the two noises a difference must clear |
| [The spread bounds the magnitude, not the direction](#the-spread-bounds-the-magnitude-not-the-direction) | the owner's 2026-09-19 ruling, the three verdicts, ties, and why the deciding quantities are factors |
| [The case that forced the ruling, and where it landed](#the-case-that-forced-the-ruling-and-where-it-landed) | Lucas-Kanade at 105 of 105 rounds, and the role bars re-taken under the ruling |
| [What changed when the rule was applied](#what-changed-when-the-rule-was-applied) | 146 rows re-judged, what moved, and the knife edge at the margin |

## How a difference is decided

Every ratio in [cuda.md](cuda.md) and [cuda-evidence.md](cuda-evidence.md) is a
**per-round paired** ratio: each round times
...
on one row of the CUDA backend the two came apart.
...
forced it is [cuda.md](cuda.md)'s. Device Lucas-Kanade at the frontend's own
...
measured cost of ignoring it is at the end of
[What changed when the rule was applied](#what-changed-when-the-rule-was-applied).
...
decision. Ties did occur in the sweeps behind these reports, all of them genuine
...
ships, and [its role bar](cuda.md#the-role-bars) reads as the comparison it is.

The rule still bites, and on this backend it currently bites one operation.
[`cornerSubPixAsync`](cuda-evidence.md#where-bincv-has-no-structural-advantage-said-plainly)
is measured losing to the whole-plane round trip it
...
number in these reports and is therefore a change of its own.
