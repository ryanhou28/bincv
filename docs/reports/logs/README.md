...
| `<bench>-cuda-launches.log` | **separate processes of one CUDA benchmark on the reference GPU**, written by `scripts/run_cuda_launches.sh`. Two are committed: `cuda_role-x86_64-cuda-launches.log` and `cuda_role_lk-x86_64-cuda-launches.log`, which the thirteen marked rows of [cuda.md](../cuda.md)'s speed table come from. The second exists separately because the Lucas-Kanade rows need a real frame sequence and the benchmark refuses to synthesize one. The header contract is described below. |
