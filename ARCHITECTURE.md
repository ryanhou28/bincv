...
| **E-41** | The FAST vector path is **x86-only**. Give it a NEON one. | [X-76](EXPERIMENTS.md) took `detectFast` from **3.9× slower than `cv::FAST` to 1.04× faster** with an AVX2 path, and `computeBrief` from 1.4× slower to **5× faster** with no intrinsics at all. aarch64 got the scalar fixes but not the vector structure. | **Everything the x86 path needs, NEON has.** The ring loads are contiguous (`vld1q_u8`), the unsigned compares are native (`vcgtq_u8`/`vcltq_u8` — no saturating-subtract trick needed, and no sign bias), and the vertical run-length accumulation is `vandq_u8` over `vaddq_u8`. The only piece without a direct equivalent is the final `movemask`, and ops/pack.hpp already carries the AND-bit-weights-then-pairwise-add idiom for exactly that. | Whether the deployment target gets the 4× the desktop just got. | X-76 | **Phase 5** |
