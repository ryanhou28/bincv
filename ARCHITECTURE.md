...
| **E-23** | `residualSums` is now **extraction-bound**: 45.4% of the kernel is `alignedWord`, the interior test, the tap identity and the masks, with **zero** counting. What does that cost look like broken down, and how much of it is addressable? | [X-40](EXPERIMENTS.md) measured it directly with a floor arm. It was **13.7%** at [D-29](#8-design-decisions) and grew to 45.4% because D-30, D-31, D-33 and X-35 made the counting ~3× faster and never touched the addressing. The whole remaining budget for count reshaping is **2.205×** and 1.069× of it is spent, so **this is where the rest of LK's speed is**. **A tax to pay first:** the per-row extraction block now exists in THREE copies (`alignedResidualSums`, `…Neon1`, `…Neon2`), and it is the thing to be changed — so factoring it is step one, not cleanup. | The shape of the aligned path, and whether the three copies collapse to one. | D-37 | **Phase 5** |
