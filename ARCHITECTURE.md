...
| **E-33** | Restructure the hot kernels to process **8 destination words at once**, plane arrays held as vectors, so AVX2 — and later CUDA — can reach them. | [X-58](EXPERIMENTS.md) localised the obstruction to **loop order, not layout**: a plane row is contiguous words, so 8 of them are one `_mm256_loadu_si256`. [X-59](EXPERIMENTS.md) priced the two operations it rests on at **≈4.7× (adder)** and **≈7.9× (popcount, beating hardware `POPCNT`)**. | **Bounded honestly.** Both ceilings landing in full would put the x86 frontend at **0.74 ms against OpenCV's 3.70 — 5×**. Ceilings here have overstated by **1.13–1.85×** measured three times ([D-37](#8-design-decisions), [D-40](#8-design-decisions), [D-41](#8-design-decisions)), which discounts to **≈3–4×**. **Memory saving: zero** — layout is not being changed, and a relayout would not change how many bits are stored anyway. **Serves the GPU port too**: word-index-as-parallel-dimension is the CUDA mapping, and bit-planes coalesce where an interleaved layout would stride. | Whether binCV is ahead rather than at parity on x86 — and whether Phase 6's GPU backend inherits the work. | D-48 | **Phase 5** |
