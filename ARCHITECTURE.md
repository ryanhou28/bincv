...
| **E-14** | Does the tracker need a border on its coarse pyramid levels — the reference's `winSize`-wide reflected pad, a cheaper replicate pad, or a keypoint policy that never places a window near a coarse-level edge? | **[X-24](EXPERIMENTS.md) promoted this from a secondary term to the DOMINANT one.** X-20 estimated the clipped coarse window at about half the four-level error; measured directly, `1/2/2/2` on `(1,0)` goes **0.8356 → 0.0010 px** when restricted to points that never clip, so clipping is essentially all of that ladder's error. Only **58 of 141 keypoints (41%)** have a 31×31 window inside all four levels at 752×480. The reference pays 1.24× per level for the border binCV declined ([deviation (ii)](#79-known-hard-problems)); a replicate pad is two mask-selects per word where reflect-101 is a per-pixel index map, and a keypoint policy costs nothing but discards 59% of the frame's candidates. **This is a footprint-versus-accuracy trade of exactly the kind CLAUDE.md's tiebreak exists for, and it must be measured rather than argued.** | Whether pyramid levels gain a border, and of what kind; and whether E-7's ladder can be chosen at all before it is settled. | **Blocks E-7's adoption**; T4.2's comparison rests on the same points | **Phase 4** |
