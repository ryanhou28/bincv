...
| **E-39** | [X-69](EXPERIMENTS.md)/[X-70](EXPERIMENTS.md)'s staging is **held off on aarch64** for `N` ∈ {1,2} at `uint32_t` — the shipped ladder's whole depth range. Write the staged NEON variants. | `residualSums` dispatches those cases to `alignedResidualSumsNeon1`/`Neon2` — [D-33](#8-design-decisions)'s tap batching and [X-40](EXPERIMENTS.md)'s window-carried accumulator, **both measured on the reference device**. The staged path calls `slicedSignedSum` per value and has neither, so taking it would trade a measured optimisation for an unmeasured one. | **Mechanical but not free.** The extraction block is identical in all of them, and [X-41](EXPERIMENTS.md) already records **three copies** of it; writing two more blind would make five. The right shape is to factor the row operands out **once** so staged/unstaged × scalar/NEON share one loader — a refactor [X-41](EXPERIMENTS.md) recommended *for maintenance* and this makes worth doing *for speed*. **It cannot be measured without the reference device**, and x86 gains nothing from it (no NEON), so it waits for a device window rather than being guessed at. | Whether the 1.46× [X-69](EXPERIMENTS.md)+[X-70](EXPERIMENTS.md) bought on x86 is available on the deployment target at all. | D-60, D-61 | **Phase 5** |
