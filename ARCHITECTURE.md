...
> **[D-2](#8-design-decisions)'s bit-plane layout, which makes binCV fast on scalar
> hardware, is precisely what stops a compiler vectorising it.** The auto-vectoriser
> and the representation want the loop nest in opposite orders.
...
rather than `WordType[N]` locals — a redesign per kernel. `-mavx2` is **not adopted**:
it would raise the minimum CPU to Haswell 2013 and buy nothing measurable.
...
| **E-33** | Restructure the hot kernels to process **8 destination words at once**, plane arrays held as vectors, so AVX2 can reach them. | [X-58](EXPERIMENTS.md) localised the obstruction to loop order, not to the arithmetic: bit-sliced adders are pure `AND`/`XOR`/`OR`, which AVX2 does **256 bits at a time** against the current 32. binCV is at **parity** with OpenCV on x86 (3.70 vs 3.70); this is what beating it would take. | **A redesign per kernel, and it needs a CEILING BEFORE AN ARM** — the discipline [X-33](EXPERIMENTS.md) established and [X-52](EXPERIMENTS.md) forgot, at a cost. Price the vector popcount and the restructured `boxSum4` first; `residualSums` is the biggest stage (67%) and the hardest, since a 31-pixel window is one word per row and the vector dimension has to come from batching rows or taps. | Whether binCV is competitive or ahead on x86 — and whether the same restructuring would help aarch64, where the NEON paths were written around the same loop order. | D-48 | **Phase 5** |
