...
| **E-46** | A VIO frontend must space NEW detections against the tracks it is already following. binCV does not: its spacing filter spaces new corners only against each other, and `examples/vio_frontend.cpp` hand-rolls the missing loop. Which shape ships? | The reference calls `applyMinDistance(corners, prevCorners, maskRadius)` on **every detect**; without it a top-up plants a fresh feature on top of a point already being tracked. Reported from outside the project, and our own example is the evidence. | Two arms: **exhaustive** `O(new × live)` float distance tests, costing no memory; and a **1-bit occupancy mask** — stamp a disc per live track, one bit test per candidate — costing one 1-bit frame (38 400 B at 640×480). **Bit-parallel is a reason to MEASURE the mask, not a reason to assume it wins**; a first estimate puts the two at the same order at the frontend's operating point. | Whether binCV's answer to detection spacing is a plain loop or a bit-plane, and whether 38 400 B buys anything. [X-93](EXPERIMENTS.md) holds the rule. | — | **Phase 5** |
| **E-47** | Does an initial-flow guess actually cut tracking time, and past what guess error does it stop helping? | `LKParams` declined the mode because "an unused mode is an untested one" — **checked against the wrong thing**: `seal_params.yaml` has the flag off, but HybVIO's `computeOpticalFlow` turns it on from `predictOpticalFlow` and seeds LK from an odometry predictor. IMU-aided tracking is the normal VIO case. | The feature ships on its own merits; what needs a rule is the **claim**. Iterations per point-level and `track` wall-clock, for a perfect guess and for guesses corrupted by 0.25/0.5/1/2/4 px, both architectures, through the existing `BINCV_LK_ITERATION_HISTOGRAM` hook. | Whether the header may claim a speed benefit at all, and over what guess-error range. [X-94](EXPERIMENTS.md) holds the rule. | D-58 | **Phase 5** |
