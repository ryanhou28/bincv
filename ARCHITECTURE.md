...
| **E-41** | `detectFast` is **3.9× slower** than `cv::FAST` and `computeBrief` **1.4× slower** than `cv::ORB::compute`, which does strictly more work. Vectorise both. | [X-76](EXPERIMENTS.md) measured them. Two scalar fixes took FAST from 16× slower to 3.9× — ring offsets hoisted out of the pixel loop, and the arc scan replaced by a bitmask contiguity test — and the remainder is **SIMD**: `cv::FAST` is vectorised and this is not. | **FAST's inner test is 16 byte comparisons against two thresholds, which is a move-mask away from being a 16-bit pair of masks** — the same shape [X-71](EXPERIMENTS.md) used for the conversion, and the contiguity test already consumes exactly those masks. `computeBrief` is 256 independent `img[a] < img[b]` comparisons per keypoint, i.e. a gather problem, and [X-61](EXPERIMENTS.md) is the standing warning about those. | Whether the descriptor family is competitive or merely correct. | X-76 | **Phase 5** |
